...
| Demo 区域 / 行为 | 生产参考（Vue） |
|------------------|-----------------|
| 单层顶栏：项目信息 + 用户区 + 管理员勾选 + 环境 / 刷新 | `CollabTableView` 顶栏与会话区合并实现 |
| 身份条 + 会话提示 | 当前用户 / JWT 角色展示 |
| **模拟管理员会话** 勾选 | `hasAdminSession` 为真时显示的配置块 |
| 项目摘要内 **简介 / 开发者 / 技术栈** | 仓库元数据或项目设置 API |
| **项目摘要 50% + 动态区 50%** 同排（窄屏纵向堆叠） | 首页首屏信息密度控制 |
| **两页轮播**：一页汇总全部「风险与阻塞」、一页仅最新「每日总结」；「更多」仅历史每日 | 首页动态区 + 风险列表数据源 |
| L3 **通栏**：仅控制台嵌入看板 | 嵌入 `DashboardView` 占满宽 |
