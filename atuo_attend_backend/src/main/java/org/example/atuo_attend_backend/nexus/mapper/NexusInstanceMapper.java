...
public interface NexusInstanceMapper {
    ...
    @Insert("""
            INSERT INTO aa_nexus_cloud_instance
              (tenant_id, account_id, instance_id, instance_name, status, instance_type, zone_id, public_ip, private_ip, os_name, memory_mb)
            VALUES
              (#{tenantId}, #{accountId}, #{instanceId}, #{instanceName}, #{status}, #{instanceType}, #{zoneId}, #{publicIp}, #{privateIp}, #{osName}, #{memoryMb})
