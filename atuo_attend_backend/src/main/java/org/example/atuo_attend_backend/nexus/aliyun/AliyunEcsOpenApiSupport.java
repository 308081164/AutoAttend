...
public final class AliyunEcsOpenApiSupport {
    ...
    public static String friendlyMessage(TeaException ex) {
        ...
        if (biz == 40302) {
            ...
            sb.append("请在阿里云 RAM 中为该密钥关联的策略中增加对应 ECS 接口权限：");
            sb.append("只读列举需 ecs:DescribeSecurityGroups、ecs:DescribeSecurityGroupAttribute；");
            sb.append("新增/修改/删除规则需 ecs:AuthorizeSecurityGroup、ecs:ModifySecurityGroupRule、ecs:RevokeSecurityGroup（入方向）及 ecs:AuthorizeSecurityGroupEgress、ecs:ModifySecurityGroupEgressRule、ecs:RevokeSecurityGroupEgress（出方向）。");
            sb.append("更新策略后可在本页重试，或在「快捷运维」云账号设置中更换具备权限的 AccessKey。");
        } else if (biz == 40400) {
            sb.append("阿里云返回资源不存在或 ID 无效，请刷新安全组列表后重试。");
        } else {
            sb.append("调用阿里云安全组接口失败。");
        }
