...
public interface QuotePriceConfigMapper {
    ...
    @Select("SELECT id, region_label AS regionLabel, price_per_day AS pricePerDay, currency, enabled FROM biz_quote_price_config ORDER BY id")
    ...
    @Insert("INSERT INTO biz_quote_price_config (region_label, price_per_day, currency, enabled) VALUES (#{regionLabel}, #{pricePerDay}, #{currency}, #{enabled})")
    @Options(useGeneratedKeys = true, keyProperty = "id")
    int insert(QuotePriceConfigRow row);

    @Update("UPDATE biz_quote_price_config SET region_label = #{regionLabel}, price_per_day = #{pricePerDay}, currency = #{currency}, enabled = #{enabled} WHERE id = #{id}")
