...
public class UiPrototypeService {
    ...
    private String buildSystemPrompt() {
        return ""
                ...
                + "\n"
                + "关于需求形态：\n"
                + "- 若输入为**自然语言需求**（如分段「PRD / 需求摘要」「AI 智能录入原文」等，且无附带功能清单表）：必须以该叙述为**唯一**主线推导页面、导航与 Tab；须落实页面划分、空间布局、组件层级与**字段声明**（中文标签与控件类型）；**禁止**臆造附录或按虚构表格行分页。\n"
                + "- 若输入**仅为**「功能清单」Markdown 表格（模块/功能点/复杂度/数量）：以表格语义归纳角色与工作流，**合并**相近能力，**禁止**「一行功能点 = 一个 Tab」机械映射；仍须落实布局分区与字段级说明（可用 Text 概括）。\n"
                + "- 若输入为完整《页面设计文档》长文（含多章节说明）：以其中**用户叙述**为主线；文中若出现附录表仅作参考，**禁止**按表行数机械生成 Tab。\n"
                + "- 若输入含旧式标记【模块1】等**显式**分段标记：可按该标记组织 Tabs，tab label 须保留【模块N】以便校验。\n"
    ...
    private String buildUserPrompt(String prompt) {
        return ""
                ...
                + "- 不要使用任何图片与外部资源\n"
                + "- 文案简洁\n"
                + "- 自然语言需求：优先还原**页面划分**与**空间布局**，并用 Text/Card 写清主要**字段标签**。**仅功能清单表**时先归纳工作流再布局，勿用表格行全文当 Tab 标题。\n"
                + "\n输出规则（务必遵守）：\n"
                + "1) 只输出一个 JSON object，不要任何解释文字/Markdown/代码块。\n"
                + "2) 若为自然语言需求导入：完全依据正文叙述组织 nodes。若为**仅功能清单表**：依据功能点语义合并场景后再组织。**禁止**用表格行全文当 Tab 标题。Tabs/面板应对应业务状态或页面分区；用 Text 体现**字段名**与布局分区标题。\n"
                + "3) 仅当需求文本中**显式**出现【模块N】标记时：用 Tabs 对应各段，label 含【模块N】，内容区用 Badge/Text 概括能力。\n"
