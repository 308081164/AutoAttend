...
public interface AiAnalysisConfigMapper {
    ...
    @Select("SELECT id, provider, api_key AS apiKey, enabled, daily_summary_enabled AS dailySummaryEnabled, model, prompt_version AS promptVersion, max_diff_chars AS maxDiffChars, created_at AS createdAt, updated_at AS updatedAt FROM aa_ai_analysis_config WHERE provider = #{provider} LIMIT 1")
    ...
    @Insert("INSERT INTO aa_ai_analysis_config (provider, api_key, enabled, daily_summary_enabled, model, prompt_version, max_diff_chars) VALUES (#{provider}, #{apiKey}, #{enabled}, #{dailySummaryEnabled}, #{model}, #{promptVersion}, #{maxDiffChars})")
