...
-- 幂等：列已存在时由 mysql --force 跳过

ALTER TABLE aa_tenant
  ADD COLUMN referrer_tenant_id BIGINT NULL COMMENT '推荐方租户 ID（被邀请注册时写入）' AFTER slug,
  ADD COLUMN invite_code_redeemed TINYINT(1) NOT NULL DEFAULT 0 COMMENT '是否已使用过邀请码兑换' AFTER referrer_tenant_id,
  ADD COLUMN member_points INT NOT NULL DEFAULT 0 COMMENT '会员积分，1 积分=1 元购买力' AFTER invite_code_redeemed,
  ADD COLUMN team_first_month_used TINYINT(1) NOT NULL DEFAULT 0 COMMENT '尝鲜版首月优惠价是否已用过' AFTER member_points;
