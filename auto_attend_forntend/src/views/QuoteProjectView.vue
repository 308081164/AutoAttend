...
export default {
  ...
  watch: {
    ...
    showCreateAgentModal (open) {
      if (open && this.form && this.form.id) {
  ...
  methods: {
    ...
    async loadAgentBackgroundAttachments () {
      ...
      try {
        const resp = await this.$http.get(`/admin/agent/quote/projects/${this.form.id}/agent-background-attachments`)
        if (resp.data && resp.data.code === 0 && resp.data.data) {
          const d = resp.data.data
    ...
    async onAgentBgFileSelected (e) {
      ...
      if (!file || !this.form || !this.form.id) return
      ...
      try {
        ...
        const resp = await this.$http.post(
          `/admin/agent/quote/projects/${this.form.id}/agent-background-attachments`,
          fd,
