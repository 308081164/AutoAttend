export default {
  app: {
    title: 'AutoAttend 管理后台（MVP）',
    adminLabel: '管理员',
    logout: '退出'
  },
  login: {
    title: '管理员登录',
    username: '用户名',
    password: '密码',
    submit: '登录',
    submitting: '登录中...',
    failed: '登录失败',
    failedBackend: '登录失败，请检查后端服务是否已启动'
  },
  collabLogin: {
    title: '项目协作登录',
    desc: '使用 commit 作者邮箱 + 默认密码 123456 登录（可由管理员重置）',
    email: '邮箱',
    password: '密码',
    placeholderEmail: 'author@example.com',
    placeholderPassword: '默认 123456',
    submit: '登录',
    submitting: '登录中...',
    failed: '登录失败',
    failedBackend: '登录失败，请检查后端服务',
    backHome: '返回首页'
  },
  dashboard: {
    statusOverview: '状态总览（Status Overview）',
    collabEntry: '项目协作',
    project: '项目（Project）',
    allProjects: '全部项目（All Projects）',
    activeCoding: '活跃开发（Active Coding）',
    inReview: '评审中（In Review）',
    reviewingOthers: '评审他人（Reviewing Others）',
    ciFixing: '修复构建（CI Fixing）',
    blocked: '阻塞（Blocked）',
    idle: '无信号（Idle）',
    recentCommits: '提交列表（最近 / Recent Commits）',
    refresh: '刷新',
    repo: '仓库（Repo）',
    commit: '提交哈希（Commit）',
    author: '作者（Author）',
    time: '时间（Time）',
    message: '提交说明（Message）',
    actions: '操作',
    viewDiff: '查看 Diff（代码变更）',
    noCommits: '暂无提交记录，可先配置 GitHub Webhook 并推送一次代码。',
    developers: '开发者统计（当前项目 / Developers by Project）',
    email: '邮箱（Email）',
    commits: '提交次数（Commits）',
    lastCommit: '最近提交时间（Last Commit）',
    diffTitle: 'Diff 详情（代码变更）',
    copyForAi: '复制给 AI 分析',
    collapse: '收起',
    loadingDiff: '加载 diff 中...',
    diffNotReady: '（diff 尚未就绪时，系统会自动重试拉取，请稍后刷新页面查看。）',
    diffLoadFailed: '加载 diff 失败',
    viewOnGitHub: '在 GitHub 上查看（需本机网络可访问 GitHub）',
    copied: '已复制到剪贴板，可粘贴到 AI 智能体进行跟进分析。',
    copyFailed: '复制失败，请手动选择下方 diff 内容复制。'
  },
  collab: {
    projectsTitle: '项目协作',
    selectProject: '选择有权限的项目进入多维表格',
    noProjects: '暂无有权限的项目。请先通过 GitHub 推送代码，系统将根据仓库自动创建项目并同步成员。',
    repoLabel: '仓库',
    backHome: '返回首页',
    loading: '加载中...'
  },
  test: {
    title: '系统能力测试',
    desc: '用于检测服务器能否拉取 Diff、AI API 与邮件服务是否可用。',
    runTest: '运行测试',
    testing: '测试中...',
    diffTitle: 'Diff 拉取（GitHub API）',
    diffDesc: '测试当前服务器能否通过 GitHub API 拉取 commit diff，与「查看 Diff」使用相同逻辑。',
    aiTitle: 'AI 分析 API',
    aiDesc: '测试配置的大模型 API 是否可达（接入后可用）。',
    emailTitle: '邮件发送',
    emailDesc: '测试邮件服务是否可用（接入后可用）。',
    result: '结果',
    latency: '耗时',
    success: '成功',
    notAvailable: '未就绪'
  },
  collabTable: {
    defaultTableName: '任务表',
    backToList: '← 项目列表',
    newRecord: '新建记录',
    loadingTable: '加载表结构中...',
    noColumns: '暂无列定义',
    operations: '操作',
    detail: '详情',
    noRecords: '暂无记录，点击「新建记录」添加',
    loadingRecords: '加载中...',
    recordDetail: '记录详情',
    fields: '字段',
    discussion: '讨论',
    attachments: '附件',
    inputComment: '输入留言...',
    send: '发送',
    uploadAttachment: '上传附件',
    newRecordTitle: '新建记录',
    createThenEdit: '创建后可点击行编辑字段。',
    create: '创建'
  }
}
