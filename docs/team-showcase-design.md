...
> **核心背景**：绝大多数客户通过手机浏览页面，移动端不是桌面端的"缩小版"，而是需要专门设计的独立模板。

### 9.1 移动端设计原则

| 原则 | 说明 |
|------|------|
| **Mobile-First** | 移动端模板独立设计，非桌面端响应式缩放 |
| **拇指友好** | 所有可点击元素最小 44x44px，间距 ≥ 12px |
| **一屏一焦点** | 每个区块不超过一屏高度，减少认知负担 |
| **即时转化** | 固定顶栏 + 内嵌 CTA 保证 3 秒内可达表单 |
| **减少滚动** | 精简内容密度，仅保留最核心的信任要素 |
...
|------|--------|--------|
| 布局方向 | 横向多列 | 纵向单列堆叠 |
| 数据卡片 | 3 列横排 | 3 列横排（保持，紧凑间距） |
| 能力/服务卡片 | 3x2 / 2x2 网格 | 2 列网格（企业/工作室）或竖排列表（个人） |
| 案例/作品 | 3 列横排 | 全宽竖排堆叠 |
| 技术栈标签 | 横向排列 | 流式换行（wrap） |
| 客户评价 | 2-3 列横排 | 全宽竖排堆叠 |
| 字号基准 | 16px | 14px（body）/ 18px（h2）/ 22px（h1） |
| 间距基准 | 24-32px | 16-20px |
| 固定顶栏 | 56px 高 | 48px 高，按钮文案缩短为「需求梳理 ▶」 |

### 9.3 企业版移动端模板

**视觉风格**：紫色渐变 Hero + 白色内容区 + 紧凑卡片
...
![企业版移动端模板效果图](template-enterprise-mobile.jpg)

### 9.4 工作室版移动端模板

**视觉风格**：温暖紫色 Hero + 紧凑服务卡片 + 竖排作品
...
![工作室版移动端模板效果图](template-studio-mobile.jpg)

### 9.5 个人版移动端模板

**视觉风格**：清新绿色 + 个人化 + 紧凑信息密度
...
│  5年+全栈开发经验...        │
├───────────────────────────┤
│  5年+ 经验 │ 80+ 项目 │ 100%│ ← 统计行（分隔线）
├───────────────────────────┤
│  专业技能                  │
│ [Vue.js] [React] [Node.js]│ ← 流式标签换行
│ [Python] [Java] [Docker] │
│ [AWS] [Go] ...            │
├───────────────────────────┤
│  服务项目                  │
...
![个人版移动端模板效果图](template-freelancer-mobile.jpg)

### 9.6 移动端技术实现策略

| 策略 | 说明 |
|------|------|
| **独立模板组件** | 每套模板创建 `ShowcaseEnterpriseMobile.vue` 等移动端专用组件，而非用 CSS 媒体查询适配桌面端组件 |
| **自动切换** | 通过 `window.innerWidth <= 768` 判断，移动端加载移动端组件，桌面端加载桌面端组件 |
| **固定顶栏** | 所有移动端模板统一使用 `position: sticky; top: 0; z-index: 100`，高度 48px |
| **触控优化** | 按钮最小高度 44px，列表项最小高度 48px，点击区域间距 ≥ 12px |
| **图片懒加载** | 案例缩略图使用 `loading="lazy"`，避免首屏加载过慢 |
| **性能优先** | 移动端模板组件使用 `defineAsyncComponent` 异步加载，不影响表单交互性能 |
...
| P0 | 前端展示区组件（桌面端） | ShowcaseSection + 3 个桌面端模板组件 |
| P0 | 前端展示区组件（移动端） | 3 个移动端专用模板组件 + 自动切换逻辑 |
| P0 | QuickQuoteLanding 集成 | 在表单上方插入展示区 + 固定顶栏 + 内嵌 CTA |
| P1 | 管理端配置页 | SystemSettingsPage 新增配置区块 |
| P1 | 模板内容编辑表单 | 结构化表单 + 动态列表 |
| P2 | 自定义 HTML 模式 | iframe 渲染 + HTML Sanitizer |
| P2 | 模板实时预览 | 管理端预览功能（桌面端 + 移动端预览） |
