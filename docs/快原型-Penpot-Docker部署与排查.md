...
| `prepare-register-profile HTTP 404` | Penpot **2.12+** 将 RPC 从 `/api/rpc/command/` 改为 **`/api/main/methods/`**；拉 `latest` 镜像时旧路径会 404 | 使用已合并的后端（**`PENPOT_RPC_PATH_STYLE=auto`** 默认先试新再试旧），或显式设 `PENPOT_RPC_PATH_STYLE=new` / `legacy` |
