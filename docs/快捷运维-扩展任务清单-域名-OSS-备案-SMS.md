...
- **安全组（实时）**：`GET .../ecs/security-groups`、`GET .../ecs/security-groups/{securityGroupId}/rules`（`DescribeSecurityGroups`、`DescribeSecurityGroupAttribute`）；`POST/PUT/DELETE .../rules` 对应新增/修改/删除（`Authorize*`、`Modify*`、`Revoke*`）。**只读**仅需 Describe 类 Action；**编辑**需额外 RAM 写权限；权限不足时接口返回 **40302** 及中文说明（含建议 Action 列表）。
- **前端**：`NexusConsoleView.vue` Tab：域名 DNS、OSS、**备案（跳转）**、短信、**安全组（读 + 编辑）**。
...
| 优先级 | 能力 | 理由 |
|--------|------|------|
| **P1** | ~~**安全组规则只读**（§13 A7）~~ **已扩展为可读可写** | 与 SSH/公网排障强相关；写操作依赖 RAM；**40302** 表示密钥权限不足。 |
