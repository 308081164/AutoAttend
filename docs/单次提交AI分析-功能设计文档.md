...
### 10.5 系统回写与配置（项目级，以当前代码为准）

- **自动化三档**（`automation_mode`）：`analysis_only` / `auto_status` / `disabled`（见《多维表格协作管理》4.11 节）。已移除 `confirm` / `suggest_only` 等旧模式描述。
- **回写范围**：仅 `auto_status` 下解析 `related_records`，且**只更新**项目调整表「当前状态」或功能清单表「开发进度」；**不更新「验收结果」**。
- **候选注入**：仅 `auto_status` 下向模型注入 `collab_records`（含 `issue_tracking` 与 `feature_backlog` 开放记录摘要）；`related_records` 输出约定含 `suggested_status`（兼容旧字段 `resolve_status`）。
- **数据落库**：`related_records` 仍保留在分析结果 `raw_response` 中；结构化建议表仍为可选后续能力。
